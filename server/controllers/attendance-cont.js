...
const getHours = (req, res, next) => {
    ...
    for (i in Object.keys(attendance)){
        ...
        if(parseInt(id)){
            ...
            if(attendance[id].fullName.toLowerCase().split(' ').join('') === req.body.user_name || "950" + req.body.user_name.slice(2) == id || attendance[id].fullName.toLowerCase().split('.').join('') === req.body.user_name){
